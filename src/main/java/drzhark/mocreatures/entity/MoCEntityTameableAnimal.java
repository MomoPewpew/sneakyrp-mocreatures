...
public class MoCEntityTameableAnimal
  ...
  public Boolean processTameInteract(EntityPlayer player, EnumHand hand) {
    ...
    if (!stack.isEmpty() && stack.getItem() == MoCItems.petamulet && stack.getItemDamage() == 0 && canBeTrappedInNet()) {
      player.setHeldItem(hand, ItemStack.EMPTY);
      if (!this.world.isRemote) {
    ...
    if (!stack.isEmpty() && getIsTamed() && getHealth() != getMaxHealth() && isMyHealFood(stack)) {
      stack.shrink(1);
      if (stack.isEmpty()) {
        player.setHeldItem(hand, ItemStack.EMPTY);
      }
  ...
  public void onLivingUpdate() {
    ...
    if (getIsFlying()) {
      ...
      if (this.motionX > 0.5D) {
        this.motionX = 0.5D;
      }
      if (this.motionY > 0.5D) {
        this.motionY = 0.5D;
      }
      if (this.motionZ > 2.5D) {
        this.motionZ = 2.5D;
      }
    }
  }
