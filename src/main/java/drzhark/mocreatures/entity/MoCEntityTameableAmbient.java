...
public class MoCEntityTameableAmbient
  ...
  public Boolean processTameInteract(EntityPlayer player, EnumHand hand) {
    ...
    ItemStack stack = player.getHeldItem(hand);
    if (stack.isEmpty()) {
      return Boolean.valueOf(super.processInteract(player, hand));
    }
    ...
    if (!this.world.isRemote && getIsTamed() && (stack
    ...
    if (stack.getItem() == MoCItems.fishnet && stack.getItemDamage() == 0 && canBeTrappedInNet()) {
      player.setHeldItem(hand, ItemStack.EMPTY);
      if (!this.world.isRemote) {
