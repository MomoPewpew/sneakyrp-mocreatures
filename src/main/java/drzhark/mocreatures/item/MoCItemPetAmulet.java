  ...
  public ActionResult<ItemStack> onItemRightClick(World world, EntityPlayer player, EnumHand hand) {
    ...
    if (!world.isRemote) {
      ...
      try {
        ...
        EntityLiving tempLiving = (EntityLiving)EntityList.createEntityByIDFromName(new ResourceLocation("mocreatures:" + this.spawnClass.toLowerCase()), world);
        if (tempLiving != null && tempLiving instanceof drzhark.mocreatures.entity.IMoCEntity) {
          ...
          if (player.getEntityWorld().spawnEntity((Entity)storedCreature)) {
            MoCMessageHandler.INSTANCE.sendToAllAround((IMessage)new MoCMessageAppear(((EntityLiving)storedCreature).getEntityId()), new NetworkRegistry.TargetPoint(
                  (player.getEntityWorld()).provider.getDimensionType().getId(), player.posX, player.posY, player.posZ, 64.0D));
