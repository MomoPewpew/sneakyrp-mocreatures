  ...
  extends MoCEntityTameableAnimal
{
  ...
  public boolean attackEntityFrom(DamageSource damagesource, float i) {
    if (super.attackEntityFrom(damagesource, i)) {
      ...
      if (entity != this && entity instanceof EntityLivingBase) {
        ...
        if (getKittyState() == 10) {
          List<Entity> list = this.world.getEntitiesWithinAABBExcludingEntity((Entity)this, getEntityBoundingBox().expand(16.0D, 6.0D, 16.0D));
        ...
        if (entityliving instanceof EntityPlayer && shouldAttackPlayers()) {
          if (getKittyState() < 2) {
  ...
  protected Entity findPlayerToAttack() {
    if (this.world.getDifficulty().getDifficultyId()  > 0 && getKittyState() != 8 && getKittyState() != 10 && getKittyState() != 15 &&
      ...
      return (Entity)getClosestTarget((Entity)this, 10.0D);
    }
  ...
  protected SoundEvent getAmbientSound() {
    if (getKittyState() == 4) {
      if (getRidingEntity() != null) {
        MoCEntityKittyBed entitykittybed = (MoCEntityKittyBed)getRidingEntity();
  ...
  public boolean processInteract(EntityPlayer player, EnumHand hand) {
    ...
    if (getKittyState() == 7 && !stack.isEmpty() && (stack
      .getItem() == Items.CAKE || stack.getItem() == Items.FISH || stack.getItem() == Items.COOKED_FISH)) {
      stack.shrink(1);
      if (stack.isEmpty()) {
        player.setHeldItem(hand, ItemStack.EMPTY);
      }
      MoCTools.playCustomSound((Entity)this, MoCSoundEvents.ENTITY_KITTY_EATING);
      setHealth(getMaxHealth());
      changeKittyState(9);
      return true;
    }
    if (getKittyState() == 11 && !stack.isEmpty() && stack.getItem() == MoCItems.woolball) {
      ...
      setKittyState(8);
      if (!this.world.isRemote) {
        EntityItem entityitem = new EntityItem(this.world, this.posX, this.posY + 1.0D, this.posZ, new ItemStack((Item)MoCItems.woolball, 1));
    ...
    if (getKittyState() == 14 && getRidingEntity() != null) {
      changeKittyState(7);
      return true;
    }
  ...
  public void setDead() {
    if (!this.world.isRemote && getKittyState() > 2 && getHealth() > 0.0F) {
      return;
    }
    super.setDead();
  }
