...
/*  60 */     if (args.length == 2) {
/*  61 */       MoCEntityWyvern moCEntityWyvern; String entityType = args[0];
/*  62 */       int type = 0;
/*     */       try {
...
/*  76 */         MoCEntityHorse moCEntityHorse = new MoCEntityHorse(player.world);
/*  77 */         moCEntityHorse.setAdult(true);
/*  78 */       } else if (entityType.equalsIgnoreCase("manticore")) {
/*  79 */         MoCEntityManticorePet moCEntityManticorePet = new MoCEntityManticorePet(player.world);
/*  80 */         moCEntityManticorePet.setAdult(true);
/*  81 */       } else if (entityType.equalsIgnoreCase("wyvern")) {
/*  82 */         moCEntityWyvern = new MoCEntityWyvern(player.world);
/*  83 */         moCEntityWyvern.setAdult(false);
/*  84 */       } else if (entityType.equalsIgnoreCase("wyvernghost")) {
/*  85 */         moCEntityWyvern = new MoCEntityWyvern(player.world);
/*  86 */         moCEntityWyvern.setAdult(false);
/*  87 */         moCEntityWyvern.setIsGhost(true);
/*     */       } else {
...
/*  97 */       moCEntityWyvern.setPosition(newPosX, newPosY, newPosZ);
/*  98 */       moCEntityWyvern.setTamed(true);
/*  99 */       moCEntityWyvern.setOwnerId(null);
/* 100 */       moCEntityWyvern.setPetName("Rename_Me");
/* 101 */       moCEntityWyvern.setType(type);
/*     */       
...
/*     */         return;
/*     */       } 
/* 110 */       player.world.spawnEntity((Entity)moCEntityWyvern);
/* 111 */       if (!player.world.isRemote) {
/* 112 */         MoCMessageHandler.INSTANCE.sendToAllAround((IMessage)new MoCMessageAppear(moCEntityWyvern.getEntityId()), new NetworkRegistry.TargetPoint(player.world.provider
/* 113 */               .getDimensionType().getId(), player.posX, player.posY, player.posZ, 64.0D));
/*     */       }
/* 115 */       MoCTools.playCustomSound((Entity)moCEntityWyvern, MoCSoundEvents.ENTITY_GENERIC_MAGIC_APPEAR);
/*     */     } else {
