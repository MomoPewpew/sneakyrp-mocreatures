...
/* 329 */         String s = EnumDyeColor.byMetadata(i).getUnlocalizedName().toLowerCase();
/* 330 */         if (s.equalsIgnoreCase("lightBlue")) s = "light_blue";
/* 331 */         MoCItems.kittybed[i] = new MoCItemKittyBed("kittybed_" + s, i);
/* 332 */         registry.register(MoCItems.kittybed[i]);
/* 333 */         if (!MoCreatures.isServer()) {
/* 334 */           ModelLoader.setCustomModelResourceLocation((Item)MoCItems.kittybed[i], 0, new ModelResourceLocation("mocreatures:" + MoCItems.kittybed[i].getUnlocalizedName().replace("item.", ""), "inventory"));
/*     */         }
/*     */       }
/*     */
/* 338 */       for (Item item : items) {
/* 339 */         registry.register(item);
/* 340 */         MoCItems.ITEMS.add(item);
/* 341 */         if (!MoCreatures.isServer()) {
					String name = "mocreatures:" + item.getUnlocalizedName().replace("item.", "");
/* 342 */           ModelLoader.setCustomModelResourceLocation(item, 0, new ModelResourceLocation(name, "inventory"));
/*     */         }
